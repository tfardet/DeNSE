...
double BaseNode::get_distance_parent() const { return dist_to_parent_; };
...
TopologicalNode::TopologicalNode(BaseWeakNodePtr parent,
    ...
    : BaseNode(position, distance_to_parent,
               distance_to_parent + parent.lock()->get_distance_to_soma())
    , parent_(parent)
...
void TopologicalNode::topological_advance() { centrifugal_order_++; }
 ...
 * @param dist_to_soma
 */
void TopologicalNode::set_first_point(const BPoint &p, double dist_to_soma)
{
...
void TopologicalNode::update_branch_and_parent(BaseNodePtr parent, BranchPtr b)
{
    // update parent
    parent_ = parent;
 ...
 * \param distance_to_parent is the distance in micrometers from the parent
 * node. \param position is the real space point of the node \param
 * BranchParentID is the relative name respect to the parent, necessary for
 * build the Id
 */
Node::Node(BaseWeakNodePtr parent, double distance_to_parent, const BPoint &pos,
